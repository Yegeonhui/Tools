...
class Augmentation:
    def __init__(self, route, FolderName, Image, Xml, n,crop_h,crop_w,gap,min_object,criteria,save_folder):
        self.route = route
    ...
    def check_object(self,Image_crop,h,w):#cropimage 좌상단 좌표
        ...
        for n in range(total_object):
            ...
            if left_top or right_bottom or left_bottom or right_top:
                ...
                # if h==y_min:
                #     if h<=y_max<=h+self.criteria:
                #         break
                # if w==x_min:
                #     if w<=x_max<=w+self.crop_w:
                #         break
                # if h+self.crop_h==y_max:
                #     if h+self.crop_h-self.criteria<=y_min<=h+self.crop_h:
                #         break
                # if w+self.crop_w==x_max:
                #     if w+self.crop_w-self.crieria<=x_min<=w+self.crop_w:
                #         break
        ...
        if len(Objects_list)>=self.min_object:
            cv2.imwrite(self.route+self.save_folder+"/"+self.Image_name+"_"+str(self.n)+".jpg",Image_crop)
...
def main(crop_h,crop_w,gap,total_object,min_object,criteria):
    ...
    for FName in ListFolder:
        ...
        save_folder="/Augmentation_File"
        os.makedirs(route + save_folder, exist_ok=True)

        for idx in range(len(Image_list)):
            Image = Image_list[idx]
            Xml = xml_list[idx]
            A = Augmentation(route, FolderName, Image, Xml, 0 ,crop_h,crop_w,gap,min_object,criteria,save_folder)  # 0은 저장할때 카운트
            A.xml_parsing()
