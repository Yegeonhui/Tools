...
class Augmentation:
    def __init__(self, route, FolderName, Image, Xml, n,crop_h,crop_w,gap,min_object):
        self.route = route
    ...
    def check_object(self,Image_crop,h,w):#cropimage 좌상단 좌표
        ...
        for n in range(total_object):
            ...
            if left_top or right_bottom or left_bottom or right_top:
                ...
                if x_min<=w:
                    x_min=w
                Objects = [object_name, x_min-w, y_min-h, x_max-w, y_max-h]
...
def main(crop_h,crop_w,gap,total_object,min_object):
    route = os.getcwd()
    ...
    for FName in ListFolder:
        ...
        for idx in range(len(Image_list)):
            ...
            Xml = xml_list[idx]
            A = Augmentation(route, FolderName, Image, Xml, 0 ,crop_h,crop_w,gap,min_object)  # 0은 저장할때 카운트
            A.xml_parsing()
...
if __name__=="__main__":
    start=time.time()
    main(crop_h=800,crop_w=1200,gap=300,total_object=5,min_object=3)
